...
class UsageMonthlyViewController: CustomUIViewController {
    
    @IBOutlet var txtDateTime: UITextField!
    @IBOutlet var chartUsageMonthly: HorizontalBarChartView!
    ...
    override func viewDidLoad() {
        ...
        txtDateTime.text = CaApplication.m_Info.dfyyyyMMStd.string(from: date)
    ...
    func setChart(_ dataArray:Array<[String:Any]>) {
        ...
        chartUsageMonthly.xAxis.setLabelCount(dataArray.count+1, force: true)
        
        let curr = BarChartDataSet(entries: currDataEntry, label: "조회월")
        curr.setColor(UIColor(named: "EG_Chart_curr")!)
        let prev = BarChartDataSet(entries: prevDataEntry, label: "전년 동월")
        ...
        // X축 간격
        chartUsageMonthly.xAxis.granularity = chartUsageMonthly.xAxis.axisMaximum / Double(dataArray.count+1)
        chartUsageMonthly.xAxis.granularityEnabled = true
        chartUsageMonthly.xAxis.labelCount = dataArray.count+1
        
        chartUsageMonthly.animate(yAxisDuration: 2.5)
        
        chartUsageMonthly.xAxis.axisMinimum = 0.0
        chartUsageMonthly.xAxis.axisMaximum = 0.0 + chartData.groupWidth(groupSpace: groupSpace, barSpace: barSpace) * Double(dataArray.count)
        ...
        switch dataArray.count {
            ...
            default:
                print("Chart: default!")
                print("dataArray length is" + String(dataArray.count))
        }
    ...
    // 월별 전기 사용량 Data 불러오는 API 호출
    func getUsageMonthly(_ year: Int, _ month: Int, _ bShowWaitDialog:Bool) {
        CaApplication.m_Engine.GetUsageOfOneMonth(CaApplication.m_Info.nSeqSite, CaApplication.m_Info.nSeqMeter, year, month, bShowWaitDialog, self)
    }
    
    // API 결과 도착했을 시 호출됨
    override func onResult(_ Result: CaResult) {
        switch Result.callback {
            case m_GlobalEngine.EG_GET_USAGE_OF_ONE_MONTH:
    ...
    @objc func donedatePicker(){
        ...
        txtDateTime.text = CaApplication.m_Info.dfyyyyMMStd.string(from: datePicker.date)
